  ...
  <div class="tabs-bar">
    ...
    <div class="tabs-outer" ref="scrollOuter" @DOMMouseScroll="handlescroll" @mousewheel="handlescroll">
      <ul class="tabs-inner" ref="scrollBody" :style="{left: tagBodyLeft + 'px'}">
        <li v-for="item in tagNavList" :key="item.name" @click="handleSelect(item)" ref="tagEle" :data-route="JSON.stringify(item)">
...
export default {
  ...
  mounted () {
    let tagList = this.$refs.tagEle
    tagList.forEach((item) => {
      console.log(JSON.parse(item.dataset.router))
    })
  }
}
</script>
