<template>
  <div class="tabs-bar">
    ...
    <div class="tabs-outer" ref="scrollOuter" @DOMMouseScroll="handlescroll" @mousewheel="handlescroll">
      ...
      <div class="tabs-inner" ref="scrollBody" :style="{left: tagBodyLeft + 'px'}">
        <span v-for="item in tagNavList" :key="item.name" @click="handleSelect(item)" ref="tagEle" :data-router="JSON.stringify(item)">
          <i :class="isCurrentTag(item) ? 'circle active' : 'circle'"></i>
          <span>{{item.meta.title}}</span>
          <a-icon type="close" class="close" v-show="item.name!== 'home'" @click.stop="handleClose(item)" />
          </s>
...
export default {
  ...
  methods: {
    ...
    handleScroll (offset) {
      ...
      console.log('1', outerWidth, bodyWidth)
      if (offset > 0) { // 向左滚动
    ...
    moveToView () {
      const outerWidth = this.$refs.scrollOuter.scrollWidth //外层盒子的可视宽度 (可视区域的宽度)
      const bodyWidth = this.$refs.scrollBody.offsetWidth // 里层盒子的实际宽度 (可视区域加上被隐藏的部分的宽度和)
      console.log(outerWidth, bodyWidth)
      // if (bodyWidth < outerWidth) { //可视区域宽度能够容下所有tag的情况
      //   this.tagBodyLeft = 0
      //   console.log("<")
      // } else {
      //   console.log('>')
