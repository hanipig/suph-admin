  ...
  <div class="tabs-bar">
    ...
    <div class="tabs-outer" ref="scrollOuter" @DOMMouseScroll="handlescroll" @mousewheel="handlescroll">
      <ul class="tabs-inner" ref="scrollBody" :style="{left: tagBodyLeft + 'px'}">
        <li v-for="item in tagNavList" :key="item.name">
          <i :class="item.name === 'home' ? 'circle active' : circle"></i>
          <span>{{item.meta.title}}</span>
          <a-icon type="close" class="close" v-show="item.name!== 'home'" />
        </li>
      </ul>
    </div>
  </div>
...
export default {
  ...
  computed: {
    tagNavList () {
      return this.$store.dispatch('app/setTagNavList', tagNavList)
    },
    // ...mapGetters([
    //   'tagNavList'
    // ])
