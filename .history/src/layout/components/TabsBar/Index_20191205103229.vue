...
export default {
  ...
  methods: {
    ...
    async moveToView (tag) {
      await const outerWidth = this.$refs.scrollOuter.offsetWidth //外层盒子的可视宽度 (可视区域的宽度)
      await const bodyWidth = this.$refs.scrollBody.offsetWidth // 里层盒子的实际宽度 (可视区域加上被隐藏的部分的宽度和)
      if (bodyWidth <= outerWidth) { //可视区域宽度能够容下所有tag的情况
        this.tagBodyLeft = 0
      } else {
        console.log('555')
      }
      console.log(bodyWidth, outerWidth, tag.offsetLeft)
    }
  },
  computed: {
    ...mapGetters([
  ...
  mounted () {
    setTimeout(async this.getTagEleByRoute(), 200)
  }
}
</script>
